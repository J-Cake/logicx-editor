...
export default class Interface extends React.Component<{ documentId: string }, { leftPanel: boolean, rightPanel: boolean, leftSize: number, rightSize: number }> {
    ...
    render() {
        ...
        } = StateMgr.get().viewport.get();

        return <section id="interface"
                        className={`${this.state.leftPanel ? 'panel-left' : ''} ${this.state.rightPanel ? 'panel-right' : ''}`}>
            <ToolBar position="top">{TopToolbar}</ToolBar>

            <ToolBar position="left">{LeftToolbar}</ToolBar>
            ...
            <ToolBar position="right">{RightToolbar}</ToolBar>

            <StatusBar>
                {{
                    left: [
                        <StatusItem label={''} icon={<i
                            className="ri-layout-left-2-fill"></i> as any}>{() => (this.setState(prev => ({leftPanel: !prev.leftPanel})), null)}</StatusItem>,
                    ...
                    right: [
                        ...
                        <StatusItem label={''} icon={<i
                            className="ri-layout-right-2-fill"></i> as any}>{() => (this.setState(prev => ({rightPanel: !prev.rightPanel})), null)}</StatusItem>,
                    ]
                }}
            </StatusBar>
        </section>;
